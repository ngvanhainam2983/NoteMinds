...
export default function ProfilePage({ user, onBack, onUserUpdate, onOpenAuth }) {
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 py-8 animate-fade-in">
            <button onClick={onBack} className="flex items-center gap-2 text-sm text-[#9496a1] hover:text-white transition-colors mb-6 group">
                                ...
                                {editing ? (
                                    <div className="flex items-center gap-2">
                                        <input autoFocus type="text" value={displayName} onChange={(e) => setDisplayName(e.target.value)}
                                            className="bg-[#0f1117] border border-[#2e3144] rounded-lg px-3 py-1.5 text-lg font-bold focus:outline-none focus:border-primary-500" />
                                    ...
                                    <>
                                        ...
                                        <button onClick={() => setEditing(true)} className="p-1.5 hover:bg-[#242736] rounded-lg text-[#9496a1] hover:text-white"><Edit3 size={14} /></button>
                                    </>
                                )}
            ...
            {/* ── Tabs ── */}
            <div className="flex border-b border-[#2e3144] mb-6 overflow-x-auto hide-scrollbar">
                <button onClick={() => { setActiveTab('overview'); setError(''); setSuccess(''); }}
                    className={`px-4 py-3 text-sm font-medium border-b-2 whitespace-nowrap transition-colors flex items-center gap-2 ${activeTab === 'overview' ? 'border-primary-500 text-primary-400' : 'border-transparent text-[#9496a1] hover:text-white'}`}>
                    <User size={16} /> Tổng quan
                </button>
                <button onClick={() => { setActiveTab('security'); setError(''); setSuccess(''); setTwoFAStep('status'); }}
                    className={`px-4 py-3 text-sm font-medium border-b-2 whitespace-nowrap transition-colors flex items-center gap-2 ${activeTab === 'security' ? 'border-primary-500 text-primary-400' : 'border-transparent text-[#9496a1] hover:text-white'}`}>
                    <Shield size={16} /> Bảo mật & Đăng nhập
                </button>
                <button onClick={() => { setActiveTab('theme'); setError(''); setSuccess(''); }}
                    className={`px-4 py-3 text-sm font-medium border-b-2 whitespace-nowrap transition-colors flex items-center gap-2 ${activeTab === 'theme' ? 'border-primary-500 text-primary-400' : 'border-transparent text-[#9496a1] hover:text-white'}`}>
                ...
                <div className="space-y-6">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        <div className="bg-[#1a1d27] border border-[#2e3144] rounded-xl p-5 hover:border-primary-500/30 transition-colors">
                            <div className="flex items-center justify-between mb-3"><span className="text-xs text-[#9496a1] uppercase tracking-wider font-medium">Gói hiện tại</span><Crown size={16} className="text-amber-400" /></div>
                            ...
                            {user.planExpiresAt && <p className="text-[11px] text-[#9496a1] mt-1">Hết hạn: {new Date(user.planExpiresAt).toLocaleDateString('vi-VN')}</p>}
                        </div>
                        <div className="bg-[#1a1d27] border border-[#2e3144] rounded-xl p-5 hover:border-primary-500/30 transition-colors">
                            <div className="flex items-center justify-between mb-3"><span className="text-xs text-[#9496a1] uppercase tracking-wider font-medium">Bảo mật</span><Shield size={16} className="text-primary-400" /></div>
                            <div className="space-y-1.5">
                                <div className="flex items-center justify-between">
                                    <span className="text-xs text-[#9496a1]">2FA (TOTP)</span>
                                    {twoFAStatus?.enabled ? <span className="flex items-center gap-1 text-xs text-green-400"><ShieldCheck size={12} /> Bật</span> : <span className="flex items-center gap-1 text-xs text-[#9496a1]"><XCircle size={12} /> Tắt</span>}
                                </div>
                                <div className="flex items-center justify-between">
                                    <span className="text-xs text-[#9496a1]">Passkey</span>
                                    {passkeys.length > 0 ? <span className="flex items-center gap-1 text-xs text-green-400"><Fingerprint size={12} /> {passkeys.length}</span> : <span className="flex items-center gap-1 text-xs text-[#9496a1]"><XCircle size={12} /> Chưa có</span>}
                                </div>
                            </div>
                        </div>
                        <div className="bg-[#1a1d27] border border-[#2e3144] rounded-xl p-5 hover:border-primary-500/30 transition-colors">
                            <div className="flex items-center justify-between mb-3"><span className="text-xs text-[#9496a1] uppercase tracking-wider font-medium">Hoạt động</span><Clock size={16} className="text-primary-400" /></div>
                            <div className="space-y-2">
                                <div><p className="text-[11px] text-[#9496a1] mb-0.5">Đăng nhập lần cuối</p><p className="text-sm font-medium">{lastLogin}</p></div>
                                {user.lastIp && <div><p className="text-[11px] text-[#9496a1] mb-0.5">IP gần nhất</p><p className="text-sm font-medium font-mono">{user.lastIp}</p></div>}
                            </div>
                        </div>
                    </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-[#1a1d27] border border-[#2e3144] rounded-xl p-6">
                        ...
                        <form onSubmit={handleChangePassword} className="space-y-4 max-w-md">
                            <div>
                                <label className="text-xs text-[#9496a1] mb-1.5 block">Mật khẩu hiện tại</label>
                                <div className="relative flex items-center">
                                    <span className="absolute left-3 text-[#9496a1]"><Lock size={16} /></span>
                                    <input type="password" value={oldPw} onChange={e => setOldPw(e.target.value)}
                                        className="w-full bg-[#0f1117] border border-[#2e3144] rounded-xl pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:border-primary-500" required />
                                </div>
                            </div>
                            <div>
                                <label className="text-xs text-[#9496a1] mb-1.5 block">Mật khẩu mới</label>
                                <div className="relative flex items-center">
                                    <span className="absolute left-3 text-[#9496a1]"><Lock size={16} /></span>
                            ...
                            <div>
                                <label className="text-xs text-[#9496a1] mb-1.5 block">Xác nhận mật khẩu mới</label>
                                <div className="relative flex items-center">
                                    <span className="absolute left-3 text-[#9496a1]"><Lock size={16} /></span>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="bg-[#1a1d27] border border-[#2e3144] rounded-xl p-6">
                            ...
                            {twoFAStep === 'status' && (
                                <div className="space-y-4">
                                    <div className={`flex items-center gap-3 p-4 rounded-xl border ${twoFAStatus?.enabled ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-[#0f1117] border-[#2e3144]'}`}>
                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${twoFAStatus?.enabled ? 'bg-emerald-500/20' : 'bg-[#242736]'}`}>
                                            {twoFAStatus?.enabled ? <ShieldCheck size={20} className="text-emerald-400" /> : <ShieldOff size={20} className="text-[#9496a1]" />}
                                        </div>
                                        <div>
                                            <p className="text-sm font-medium">{twoFAStatus?.enabled ? 'Đã bật' : 'Chưa bật'}</p>
                                            <p className="text-xs text-[#9496a1]">{twoFAStatus?.enabled ? `Còn ${twoFAStatus.recoveryCodesRemaining} mã khôi phục` : 'Dùng Google Auth/Authy'}</p>
                                        </div>
                                    </div>

                                    {twoFAStatus?.enabled ? (
                                        <div className="flex gap-2">
                                            <button onClick={() => { setTwoFAStep('regen'); setRegenPassword(''); }} className="flex-1 py-2bg-[#242736] bg-[#2e3144] hover:bg-[#3e4154] rounded-lg text-xs font-medium transition-colors flex items-center justify-center gap-1">
                            ...
                            {twoFAStep === 'setup' && setupData && (
                                <div className="space-y-4">
                                    ...
                                    <div className="bg-[#0f1117] border border-[#2e3144] rounded-xl p-3 flex items-center gap-2">
                                        <code className="flex-1 text-xs font-mono text-primary-400 break-all">{setupData.secret}</code>
                                        <button onClick={() => navigator.clipboard.writeText(setupData.secret)} className="p-1.5 hover:bg-[#242736] rounded-lg text-[#9496a1]"><Copy size={14} /></button>
                                    </div>
                                    <input type="text" maxLength={6} value={totpCode} onChange={e => setTotpCode(e.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="000000"
                                        className="w-full bg-[#0f1117] border border-[#2e3144] rounded-xl px-4 py-3 text-center text-lg font-mono tracking-[0.5em] focus:outline-none focus:border-primary-500" autoFocus />
                                    <div className="flex gap-2">
                                        <button onClick={() => { setTwoFAStep('status'); setSetupData(null); }} className="flex-1 py-2 bg-[#242736] hover:bg-[#2e3144] rounded-xl text-sm font-medium">Huỷ</button>
                            ...
                            {twoFAStep === 'disable' && (
                                <div className="space-y-3">
                                    ...
                                    <input type="password" value={disablePassword} onChange={e => setDisablePassword(e.target.value)} placeholder="Nhập mật khẩu" className="w-full bg-[#0f1117] border border-[#2e3144] rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-red-500" />
                                    <div className="flex gap-2">
                                        <button onClick={() => { setTwoFAStep('status'); setDisablePassword(''); }} className="flex-1 py-2 bg-[#242736] rounded-xl text-sm font-medium">Huỷ</button>
                            ...
                            {twoFAStep === 'regen' && (
                                <div className="space-y-3">
                                    ...
                                    <input type="password" value={regenPassword} onChange={e => setRegenPassword(e.target.value)} placeholder="Nhập mật khẩu" className="w-full bg-[#0f1117] border border-[#2e3144] rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-amber-500" />
                                    <div className="flex gap-2">
                                        <button onClick={() => { setTwoFAStep('status'); setRegenPassword(''); }} className="flex-1 py-2 bg-[#242736] rounded-xl text-sm font-medium">Huỷ</button>
                        ...
                        <div className="bg-[#1a1d27] border border-[#2e3144] rounded-xl p-6">
                            ...
                            <div className="space-y-3 mb-4">
                                {passkeys.length === 0 ? (
                                    <p className="text-sm text-[#9496a1]">Chưa có passkey nào. Thêm passkey để đăng nhập nhanh qua vân tay, khuôn mặt hoặc USB token.</p>
                                ) : (
                                    passkeys.map(pk => (
                                        <div key={pk.id} className="flex items-center gap-3 bg-[#0f1117] border border-[#2e3144] rounded-lg px-3 py-2.5">
                                            <Fingerprint size={16} className="text-primary-400 shrink-0" />
                                            <div className="flex-1 min-w-0">
                                                <p className="text-sm font-medium truncate">{pk.name}</p>
                                                <p className="text-[10px] text-[#9496a1] truncate">{pk.deviceType === 'multiDevice' ? 'Đa thiết bị' : 'Đơn thiết bị'}</p>
                                            </div>
                                            <button onClick={() => handleDeletePasskey(pk.id)} className="p-1.5 hover:bg-red-500/10 rounded-md text-[#9496a1] hover:text-red-400"><Trash2 size={14} /></button>
                                        </div>
                                    ))
                                )}
                            </div>
                            ...
                            <div className="flex gap-2">
                                <input type="text" value={passkeyName} onChange={e => setPasskeyName(e.target.value)} placeholder="Tên (VD: MacBook)"
                                    className="flex-1 bg-[#0f1117] border border-[#2e3144] rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-primary-500" />
                                <button onClick={handleRegisterPasskey} disabled={loading} className="px-4 py-2 bg-[#242736] hover:bg-[#2e3144] rounded-lg text-sm font-medium flex items-center gap-1.5 border border-[#2e3144]">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                        {Object.entries(THEMES).map(([key, t]) => (
                            <button
                                ...
                                className={`flex flex-col items-center gap-3 p-4 rounded-xl border-2 transition-all hover:scale-105 ${currentTheme === key ? 'border-primary-500 bg-primary-600/10 shadow-lg shadow-primary-500/10' : 'border-[#2e3144] bg-[#0f1117] hover:border-[#3e4154]'}`}
                            >
